...
| 3 | Staged files | `git ls-files` after `git add -A` | Pass — 69 project files only, including this report |
...
| 10 | Independent history and author | `git log` and repository-local identity inspection | Pass — all sixteen existing commits on main are authored by Pasindu Ranasinghe. Fifteen use the supplied UNSW email; the GitHub merge uses the same author's GitHub noreply address. The staged release uses the supplied UNSW email. No parent-workspace history is inherited. |
