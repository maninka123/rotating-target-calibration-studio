...
| 10 | Independent history and author | `git log --all` and repository-local identity inspection | Pass — all seven existing project commits are authored solely by Pasindu Ranasinghe using the supplied UNSW email address; the staged release uses the same local identity |
