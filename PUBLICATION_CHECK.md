...
| 3 | Staged files | `git ls-files` after `git add -A` | Pass — 61 project files only, including this report |
...
| 10 | Independent history and author | `git log --all` and repository-local identity inspection | Pass — all six existing project commits are authored solely by Pasindu Ranasinghe using the supplied UNSW email address; the staged release uses the same local identity |
