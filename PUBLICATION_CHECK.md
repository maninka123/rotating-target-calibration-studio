...
| 10 | Independent history and author | `git log` and repository-local identity inspection | Pass — all twelve existing commits on main are authored by Pasindu Ranasinghe. Eleven use the supplied UNSW email; the GitHub merge uses the same author's GitHub noreply address. The staged release uses the supplied UNSW email. No parent-workspace history is inherited. |
