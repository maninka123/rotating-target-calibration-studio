...
test('static multi-sensor sweep labels and downloaded package preserve the reviewed run', async ({ page }) => {
  ...
  expect(new Set(saved.records.map((row: { sensorName: string }) => row.sensorName))).toEqual(new Set(['FLIR Blackfly S', 'FLIR Blackfly S — 20 ms rolling']))
  ...
  expect(saved.pairwiseOffsets[0]).toMatchObject({ fromSensorName: 'FLIR Blackfly S', toSensorName: 'FLIR Blackfly S — 20 ms rolling' })
