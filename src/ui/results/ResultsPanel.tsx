    ...
    <Panel number={6} title="Estimation and results" className="results-panel">
      ...
      <div className="comparison-grid" data-testid="estimator-results">
        {props.sensors.flatMap((sensor) => (props.estimates[sensor.instanceId]?.results ?? []).map((result) => (
          <EstimateCard key={`${sensor.instanceId}-${result.estimator}`} sensor={sensor} frame={props.estimates[sensor.instanceId].frame} result={result} config={props.config} />
        )))}
...
function EstimateCard({ sensor, frame, result, config }: { sensor: PlacedSensor, frame: SampleFrame, result: EstimateResult, config: SimulationConfig }) {
  const plot = useRef<HTMLCanvasElement>(null)
  const cost = useRef<HTMLCanvasElement>(null)
  useEffect(() => {
    if (plot.current) drawSamples(plot.current, frame, config.target, result)
    if (cost.current) drawCost(cost.current, result)
  }, [frame, result, config.target])
  const exportPng = () => {
    const link = document.createElement('a')
    link.download = `${sensor.name}-${result.estimator}-estimate.png`.replaceAll(' ', '-')
  ...
  return (
    <article className="estimate-card">
      ...
      {result.estimator === 'geometric' && <p className="method-note">Curvature uncertainty approaches numerical zero for dense regular sampling and is not meaningful in that regime.</p>}
