...
export function ScenePanel(props: Props) {
  ...
  return (
    <Panel number={3} title="3D scene" className="scene-panel">
      <Suspense fallback={<div className="scene-wrap scene-loading">Loading 3D renderer…</div>}><SceneViewport {...props} showDimensions={showDimensions} /></Suspense>
      <div className="transport">
        ...
        <label className="check"><input type="checkbox" checked={showDimensions} onChange={(event) => setShowDimensions(event.target.checked)} /> Show dimensions</label>
      </div>
    </Panel>
  )
}
